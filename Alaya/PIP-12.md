...
- 修改方案
  ...
  在升级提案通过的区块统一将锁仓错账账户金额做调整，同时在使用锁仓金额委托和质押时将严格通过锁仓合约金额验证接口对金额做验证。
  对于已经使用多出的锁仓金额进行委托或质押的账户，按以下规则统一处理：
  
  1. 优先从账户委托关系中扣减金额，原则为按节点分成比例由小到大，相同分成比例按节点ID由小到大，直到将多的金额扣除完毕
  
  2. 如果委托金额不足以扣除，将对账户质押的节点的质押金做扣除，如果有多个质押节点，优先按最近质押的节点扣除
  
  3. 如果节点被扣除质押金后不足最低质押金额，将强制对节点解除质押，质押金最多锁定一个结算周期
